...
class _ProductionTrackingMapState extends State<ProductionTrackingMap> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _harborZones = zones;
          _harborPOIs = pois;
          _catchPolygons = catchZones.map((z) => {
            'id': z.id,
            'name': z.name,
  ...
  Widget build(BuildContext context) {
    ...
    double rotation = 0;
    if (widget.isMinimized || context.watch<TrackingMinimizeProvider>().showCompass) {
      // Mode navigasi: rotate ke arah tujuan
      final lat1 = vesselPosition.latitude * pi / 180;
      final lat2 = harborPosition.latitude * pi / 180;
      final dLon = (harborPosition.longitude - vesselPosition.longitude) * pi / 180;
      ...
      children: [
        FlutterMap(
          ...
          options: MapOptions(
            ...
            initialZoom: widget.isMinimized ? 16 : (context.read<TrackingMinimizeProvider>().showCompass ? 16 : 11),
          ...
          children: [
            ...
            if (_showCatchZones && !widget.isMinimized) _buildCatchPolygonsLayer(),
  ...
  Widget _buildVesselTrail(LatLng vesselPosition) {
    ...
    if (_showCatchZones && widget.selectedCatchZoneName != null && _catchPolygons.isNotEmpty) {
      final filteredZones = _catchPolygons.where((zone) => 
        zone['name'].toString().toLowerCase() == 
        widget.selectedCatchZoneName!.toLowerCase()
      ).toList();
    ...
    return Positioned(
      ...
      child: Column(
        children: [
          ...
          SizedBox(height: spacing * 2),
          _buildControlButton(
            icon: context.watch<TrackingMinimizeProvider>().showCompass ? Icons.navigation : Icons.explore,
            onTap: () {
              ...
              if (newValue) {
                ...
                final dLon = (harborPos.longitude - vesselPos.longitude) * pi / 180;
                final y = sin(dLon) * cos(lat2);
                final x = cos(lat1) * sin(lat2) - sin(lat1) * cos(lat2) * cos(dLon);
            ...
            tooltip: context.watch<TrackingMinimizeProvider>().showCompass ? 'Mode Normal' : 'Mode Navigasi',
            size: buttonSize,
            iconSize: iconSize,
