...
class NahkodaTrackingButton extends StatelessWidget {
  ...
  void _handleTripPreparation(BuildContext context) async {
    ...
    try {
      ...
      // 1. menunggu_dokumen -> MySchedulesScreen
      if (statusLower == 'menunggu_dokumen') {
        print(
          '🔵 [NahkodaButton] Status menunggu_dokumen -> MySchedulesScreen',
        );
        if (context.mounted) {
    ...
    return FutureBuilder<Map<String, dynamic>?>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.done && snapshot.hasData && snapshot.data != null) {
          ...
          else if (status == 'menunggu_dokumen' || status == 'menunggu_izin' || status == 'pending' || status == 'aktif') {
