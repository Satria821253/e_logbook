...
class Step7SuratSehat extends StatefulWidget {
  final VoidCallback onNext;
  const Step7SuratSehat({Key? key, required this.onNext}) : super(key: key);
  @override
  State<Step7SuratSehat> createState() => _Step7SuratSehatState();
}

class _Step7SuratSehatState extends State<Step7SuratSehat> {
  File? _selectedFile;
      ...
      if (mounted) {
        if (result['success'] == true) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Surat Keterangan Sehat berhasil diupload!'), backgroundColor: Colors.green),
          );
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                ...
                const Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Surat Keterangan Sehat', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                      SizedBox(height: 4),
                      Text('Medical Certificate', style: TextStyle(color: Colors.white70, fontSize: 14)),
                    ],
                  ),
                ),
              ],
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Petunjuk Upload', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green[900], fontSize: 15)),
                      const SizedBox(height: 8),
                      Text('• Dari dokter atau rumah sakit\n• Masih berlaku (max 6 bulan)\n• Mencantumkan hasil pemeriksaan', style: TextStyle(color: Colors.green[800], fontSize: 13, height: 1.5)),
                    ],
                  ),
                ),
              ],
          ...
          const SizedBox(height: 24),
          FilePickerWidget(label: 'Upload Surat Keterangan Sehat *', onFilePicked: (file) => setState(() => _selectedFile = file)),
          const SizedBox(height: 24),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Nomor Surat *', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.black87)),
