...
class Step6BST extends StatefulWidget {
  ...
  const Step6BST({Key? key, required this.onNext}) : super(key: key);
  @override
  State<Step6BST> createState() => _Step6BSTState();
}

class _Step6BSTState extends State<Step6BST> {
  File? _selectedFile;
  ...
  Future<void> _uploadDocument() async {
    ...
    if (_nomorController.text.trim().isEmpty) {
      _showError('Nomor sertifikat BST harus diisi');
      return;
    }
      ...
      if (mounted) {
        if (result['success'] == true) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('BST berhasil diupload!'), backgroundColor: Colors.green),
          );
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                ...
                const Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('BST', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                      SizedBox(height: 4),
                      Text('Basic Safety Training', style: TextStyle(color: Colors.white70, fontSize: 14)),
                    ],
                  ),
                ),
              ],
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Petunjuk Upload', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red[900], fontSize: 15)),
                      const SizedBox(height: 8),
                      Text('• Sertifikat BST masih berlaku\n• Foto harus jelas dan lengkap\n• Pastikan nomor dan tanggal terlihat', style: TextStyle(color: Colors.red[800], fontSize: 13, height: 1.5)),
                    ],
                  ),
                ),
              ],
          ...
          const SizedBox(height: 24),
          FilePickerWidget(label: 'Upload BST *', onFilePicked: (file) => setState(() => _selectedFile = file)),
          const SizedBox(height: 24),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Nomor Sertifikat BST *', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.black87)),
              const SizedBox(height: 12),
              TextField(
                controller: _nomorController,
                decoration: InputDecoration(
                  hintText: 'Masukkan nomor sertifikat BST',
                  prefixIcon: Icon(Icons.numbers, color: Colors.red[600]),
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                  focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.red[600]!, width: 2)),
                  filled: true,
