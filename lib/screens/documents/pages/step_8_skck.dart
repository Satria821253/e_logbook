...
class Step8SKCK extends StatefulWidget {
  ...
  const Step8SKCK({Key? key, required this.onNext}) : super(key: key);
  @override
  State<Step8SKCK> createState() => _Step8SKCKState();
}

class _Step8SKCKState extends State<Step8SKCK> {
  File? _selectedFile;
  ...
  Future<void> _uploadDocument() async {
    ...
    if (_nomorController.text.trim().isEmpty) {
      _showError('Nomor SKCK harus diisi');
      return;
    }
      ...
      if (mounted) {
        if (result['success'] == true) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('SKCK berhasil diupload!'), backgroundColor: Colors.green),
          );
          
          // Update SharedPreferences untuk status dokumen
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                ...
                const Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('SKCK', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                      SizedBox(height: 4),
                      Text('Police Certificate', style: TextStyle(color: Colors.white70, fontSize: 14)),
                    ],
                  ),
                ),
              ],
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Petunjuk Upload', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue[900], fontSize: 15)),
                      const SizedBox(height: 8),
                      Text('• SKCK dari Polres/Polda\n• Masih berlaku (max 6 bulan)\n• Foto harus jelas', style: TextStyle(color: Colors.blue[800], fontSize: 13, height: 1.5)),
                    ],
                  ),
                ),
              ],
          ...
          const SizedBox(height: 24),
          FilePickerWidget(label: 'Upload SKCK *', onFilePicked: (file) => setState(() => _selectedFile = file)),
          const SizedBox(height: 24),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Nomor SKCK *', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.black87)),
              const SizedBox(height: 12),
              TextField(
                controller: _nomorController,
                decoration: InputDecoration(
                  hintText: 'Masukkan nomor SKCK',
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: _isUploading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Text('Selesai', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
            ),
          ),
        ],
      ),
