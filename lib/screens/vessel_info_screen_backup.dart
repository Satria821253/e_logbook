...
class _VesselInfoScreenState extends State<VesselInfoScreen> {
  ...
  // Fuel data controllers (for Nahkoda)
  ...
  // Ice data controllers (for ABK)
  final _jumlahEsController = TextEditingController();
  final _hargaPerBalokController = TextEditingController();
  final _totalHargaEsController = TextEditingController();
  DateTime? _tanggalPengisianEs;
  final _lokasiPengisianEsController = TextEditingController();
  final _keteranganEsController = TextEditingController();
  String? _buktiEsPath;
      ...
      if (mounted) {
        if (vesselData == null || vesselData['kapal'] == null) {
          ...
          setState(() {
            ...
            print('🚢 Nahkoda data: ${vesselData['nahkoda']}');
          });
        }
      }
    } catch (e) {
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: Icon(Icons.pending, size: 40, color: Colors.orange),
              ),
              SizedBox(height: 20),
              Text(
  ...
  void dispose() {
    ...
    _jumlahEsController.dispose();
    _hargaPerBalokController.dispose();
    _totalHargaEsController.dispose();
    _lokasiPengisianEsController.dispose();
    _keteranganEsController.dispose();
    super.dispose();
  }
  ...
  void _calculateTotalHargaEs() {
    final jumlah = double.tryParse(_jumlahEsController.text) ?? 0;
    final harga = double.tryParse(_hargaPerBalokController.text) ?? 0;
    final total = jumlah * harga;
    _totalHargaEsController.text = total.toStringAsFixed(0);
  }
  ...
  Future<void> _saveVesselInfo() async {
    ...
    if (_source == 'document-completion') {
      final roleDocsComplete = _roleDocuments.every(
        (doc) => doc['hasFile'] == true,
      );
      if (!roleDocsComplete) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Sertifikat ${_userRole == 'NAHKODA' ? 'Nahkoda' : 'Crew'} harus diupload',
            ),
            backgroundColor: Colors.orange,
          ),
        );
    ...
    try {
        ...
        for (var doc in _documentData!) {
          if (doc['hasFile'] == true) {
            ...
            final result = await DocumentService.uploadDocument(
              ...
              nomorDokumen: doc['requireNumber'] == true
                  ? doc['nomorDokumen']
                  : null,
              tanggalBerlaku: doc['requireDate'] == true
                  ? doc['tanggalBerlaku']
                  : null,
              filePath: doc['filePath']!,
              keterangan: doc['keterangan']?.isNotEmpty == true
                  ? doc['keterangan']
                  : null,
            );
      ...
      if ((roleUpper == 'NAHKODA' || roleUpper == 'NAKHODA')) {
        ...
        final fuelResult = await VesselService().uploadBahanBakar(
          ...
          lokasiPengisian: _lokasiPengisianController.text.isNotEmpty
              ...
              : null,
          keterangan: _keteranganBBMController.text.isNotEmpty
              ? _keteranganBBMController.text
              : null,
      ...
      // Upload data es ke API (hanya untuk CREW)
      if (roleUpper == 'CREW') {
        // Validate ice data
        if (_tanggalPengisianEs == null) {
          throw Exception('Tanggal pengisian es harus diisi');
        }

        final jumlahEs = double.tryParse(_jumlahEsController.text);
        final hargaPerBalok = double.tryParse(_hargaPerBalokController.text);
        final totalHargaEs = double.tryParse(_totalHargaEsController.text);

        if (jumlahEs == null ||
            hargaPerBalok == null ||
            totalHargaEs == null) {
          throw Exception('Data es tidak valid');
        }

        print('📤 Uploading ice data...');
        final iceResult = await VesselService().uploadIceData(
          jenisEs: 'Balok',
          jumlah: jumlahEs,
          hargaPerUnit: hargaPerBalok,
          totalHarga: totalHargaEs,
          tanggalPembelian: _tanggalPengisianEs!.toIso8601String(),
          lokasiPembelian: _lokasiPengisianEsController.text.isNotEmpty
              ? _lokasiPengisianEsController.text
              : null,
          keterangan: _keteranganEsController.text.isNotEmpty
              ? _keteranganEsController.text
              : null,
          buktiFilePath: _buktiEsPath,
        );
        print('📥 Ice upload result: ${iceResult['success']}');
      }
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: SingleChildScrollView(
          ...
          children: [
            SizedBox(
              height: ResponsiveHelper.height(context, mobile: 30, tablet: 40),
            ),
            _buildHeaderGradient(),
            SizedBox(
              height: ResponsiveHelper.height(context, mobile: 16, tablet: 24),
            ),
            Padding(
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    if (_userRole.toUpperCase() == 'NAHKODA' ||
                        _userRole.toUpperCase() == 'NAKHODA')
                      _buildSupplySection(),
                    if (_userRole.toUpperCase() == 'NAHKODA' ||
                        _userRole.toUpperCase() == 'NAKHODA')
                      SizedBox(
  ...
  Widget _buildDocumentSummary() {
    final uploadedDocs = _documentData!
        .where((doc) => doc['hasFile'] == true)
        .toList();
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          if (nomorController.text.isEmpty ||
                              nomorController.text.length < 3 ||
                              selectedDate == null) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  'Nomor dokumen dan tanggal berlaku wajib diisi',
                                ),
                                backgroundColor: Colors.orange,
                              ),
                            );
                ...
                TextFormField(
                  ...
                  validator: (value) {
                    if (value?.isEmpty ?? true)
                      return 'Hitung otomatis dari jumlah x harga';
                    return null;
                  },
                ),
                SizedBox(height: 16),
        ...
        children: [
          ...
          Padding(
            ...
            child: Column(
              children: [
                TextFormField(
                  controller: _jumlahEsController,
                  decoration: InputDecoration(
                    labelText: 'Jumlah Es *',
                    hintText: 'Masukkan jumlah balok es',
                    suffixText: 'Balok',
                  ...
                  keyboardType: TextInputType.numberWithOptions(decimal: true),
                  onChanged: (value) => _calculateTotalHargaEs(),
                  validator: (value) {
                    if (value?.isEmpty ?? true) return 'Wajib diisi';
                    if (double.tryParse(value!) == null) return 'Harus angka';
                    return null;
                ...
                SizedBox(height: 16),
                TextFormField(
                  controller: _hargaPerBalokController,
                  decoration: InputDecoration(
                    labelText: 'Harga Per Balok *',
                    hintText: 'Masukkan harga per balok',
                    prefixText: 'Rp ',
                    prefixIcon: Icon(Icons.attach_money, color: Colors.green),
                    border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(12),
                ...
                SizedBox(height: 16),
                TextFormField(
                  controller: _totalHargaEsController,
                  decoration: InputDecoration(
                    labelText: 'Total Harga *',
                    prefixText: 'Rp ',
                    prefixIcon: Icon(Icons.payments, color: Colors.purple),
                ...
                InkWell(
                  onTap: () async {
                    ...
                    if (date != null) {
                      setState(() => _tanggalPengisianEs = date);
                    }
                  },
                  child: InputDecorator(
                    decoration: InputDecoration(
                    ...
                    child: Text(
                      _tanggalPengisianEs != null
                          ? '${_tanggalPengisianEs!.day}/${_tanggalPengisianEs!.month}/${_tanggalPengisianEs!.year}'
                          : 'Pilih tanggal pengisian',
                      style: TextStyle(
                        color: _tanggalPengisianEs != null
                            ? Colors.black
                            : Colors.grey,
                      ),
                    ),
                ...
                SizedBox(height: 16),
                TextFormField(
                  controller: _lokasiPengisianEsController,
                  decoration: InputDecoration(
                    labelText: 'Lokasi Pengisian',
                ...
                SizedBox(height: 16),
                TextFormField(
                  controller: _keteranganEsController,
                  decoration: InputDecoration(
                    labelText: 'Keterangan',
                    hintText: 'Tambahkan keterangan',
                    prefixIcon: Icon(Icons.note, color: Colors.grey),
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_buktiEsPath != null) ...[
                        SizedBox(height: 8),
                        Text(
                          _buktiEsPath!.split('/').last,
                          style: TextStyle(
                            fontSize: 12,
                            color: Colors.grey[600],
                          ),
                          ...
                          Expanded(
                            child: OutlinedButton.icon(
                              onPressed: () async {
                                ...
                                if (image != null) {
                                  setState(() => _buktiEsPath = image.path);
                                }
                              },
                              icon: Icon(Icons.camera_alt, size: 18),
                              label: Text('Kamera'),
