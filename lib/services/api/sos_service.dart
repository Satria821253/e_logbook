...
class SosService {
  ...
  /// Cancel SOS alert (resolve emergency)
  ///
  /// Parameters:
  /// - note: Optional message for cancellation reason
  static Future<Map<String, dynamic>> cancelSosAlert({String? note}) async {
    try {
      print('✅ [SOS CANCEL] Starting SOS cancellation...');
      ...
      print('🚢 [SOS CANCEL] Kapal ID: $vesselId');

      // Get current location
      print('📍 [SOS CANCEL] Getting current location...');
      Position position = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high,
      );

      print('📍 [SOS CANCEL] Location: ${position.latitude}, ${position.longitude}');

      // Prepare cancellation message
      final message = note?.trim().isEmpty ?? true
          ? 'Status darurat telah dibatalkan. Kapal dalam kondisi aman.'
          : note!.trim();
      ...
      final requestData = {
        ...
        'location': {'lat': position.latitude, 'lng': position.longitude},
        'note': message,
        'action': 'cancel', // Parameter untuk membedakan cancel dari alert
      };

      print('✅ SOS Cancel Request:');
      print('   URL: $baseUrl/api/mobile/sos');
      print('   Data: $requestData');

      // Send POST request (same endpoint, different action)
      final response = await _dio.post(
        '$baseUrl/api/mobile/sos',
        options: Options(
          headers: {
      ...
      print('✅ SOS Cancel Response: ${response.data}');
      ...
      print('❌ SOS CANCEL DioException: ${e.message}');
      print('❌ Response: ${e.response?.data}');
      print('❌ Status: ${e.response?.statusCode}');
      ...
      if (e.response != null) {
        ...
        final message = responseData is Map
            ? (responseData['message'] ?? 'Gagal membatalkan sinyal darurat')
            : 'Gagal membatalkan sinyal darurat';
        throw Exception(message);
      } else if (e.type == DioExceptionType.connectionTimeout) {
        throw Exception('Koneksi timeout. Periksa koneksi internet Anda.');
      } else if (e.type == DioExceptionType.receiveTimeout) {
        ...
        throw Exception('Gagal membatalkan SOS: ${e.message}');
      }
    } catch (e) {
      print('❌ Unexpected error: $e');
      throw Exception('Terjadi kesalahan: $e');
